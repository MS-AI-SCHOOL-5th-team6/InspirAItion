...
async def main():
    ...
    try:
        ...
        if curation_results.get("error"):
            print(f"큐레이션 생성 중 오류 발생: {curation_results.get('message')}")
            return

        print("\n=== 큐레이션 결과 ===")
        print(f"큐레이션 스타일: {curation_results.get('style')}")
        print(f"내용: {curation_results.get('content')}")
        for idx, reference in enumerate(curation_results.get('references', []), 1):
