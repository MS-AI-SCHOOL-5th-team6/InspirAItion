...
class UnifiedInput(BaseModel):
    ...
    style: Optional[str] = Field(default=None, description="Curation style")
...
class AzureAIAssistant:
    ...
    async def _wait_for_completion(self, thread_id: str, run_id: str, timeout: int = 30) -> bool:
        ...
        while True:
            ...
            if run.status == "completed":
                ...
                self.logger.info("Handling requires_action status...")
                await self._handle_requires_action(thread_id, run_id)
                continue

            if time.time() - start_time > timeout:
                self.logger.warning("Timeout reached. Run did not complete.")
                return False

            await asyncio.sleep(1)  
            # async 사용을 위해 time.sleep 대신 asyncio.sleep 사용
    ...
    def _wait_for_completion(self, thread_id: str, run_id: str, timeout: int = 30) -> bool:
        ...
        while True:
            ...
            if run.status == "completed":
                ...
                self.logger.info("Run requires additional action. Handling it...")
                self._handle_requires_action(thread_id, run_id)
                continue
    ...
    def _handle_requires_action(self, thread_id: str, run_id: str):
        # Handle requires_action status
        self.logger.info("Handling requires_action status...")
        # Implement logic to handle requires_action status
        # For example, you might need to submit additional inputs or take specific actions
        # Here, we will submit a message to the thread to continue the run
        self.client.beta.threads.messages.create(
            thread_id=thread_id,
            role="user",
            content="Please continue the run."
        )
...
class MetMuseumTool(Tool):
    def __init__(self, client: AzureOpenAI):
        ...
        self.search_assistant = AzureAIAssistant(client)
    ...
    async def execute(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        elif inputs.get("operation") == "curation":
            return await self.create_curation(UnifiedInput(**inputs))
        else:
        ...
        return [
            ...
            },
            {
                ...
                "function": {
                    ...
                    "description": "Create a curated list of artworks based on a given style",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "style": {"type": "string", "description": "Curation style"},
                            "query": {"type": "string", "description": "Search query for artworks"}
                        },
    ...
    async def search_artworks(self, input: UnifiedInput) -> dict:
        try:
            ...
            # 검색 결과를 10개로 제한
            met_result['objectIDs'] = met_result.get('objectIDs', [])[:10]
            ...
            web_results = await self.search_assistant.search_web({
                "operation": "search",
                "query": f"art history {input.query} analysis context",
                "has_images": True
            })
    ...
    async def _analyze_artwork(self, artwork_data: dict) -> dict:
        try:
            ...
            completion = self.search_assistant.client.chat.completions.create(
    ...
    async def create_curation(self, input: UnifiedInput) -> dict:
        try:
            # 큐레이션 생성 로직
            curation_style = input.style
            query = input.query

            # 작품 검색
            search_results = await self.search_artworks(input)

            if search_results.get("error"):
                return {"error": True, "message": "Artwork search failed"}

            met_results = search_results.get("met_results", {})
            object_ids = met_results.get("objectIDs", [])
            ...
            for object_id in object_ids:
                ...
                artwork_details = await self.get_artwork_details(UnifiedInput(**details_params))

                if not artwork_details.get("error"):
                    art_reference = ArtReference(
                        title=artwork_details.get('title'),
                        artist=artwork_details.get('artistDisplayName'),
                        period=artwork_details.get('period'),
                        medium=artwork_details.get('medium'),
                        description=artwork_details.get('analysis', {}).get('style_analysis', '분석 정보 없음'),
                        url=artwork_details.get('objectURL')
                    )
                    art_references.append(art_reference)

            # 큐레이션 결과 생성
            curation_result = CurationResult(
                style=curation_style,
                content=f"큐레이션 스타일: {curation_style}, 검색어: {query}",
...
async def main():
    ...
    try:
        ...
        print("큐레이션 생성 중...")
        curation_results = await met_tool.execute(curation_params)

        if isinstance(curation_results, dict) and curation_results.get("error"):
            print(f"큐레이션 생성 중 오류 발생: {curation_results.get('message')}")
            return

        print("\n=== 큐레이션 결과 ===")
        print(f"큐레이션 스타일: {curation_results.style}")
        print(f"내용: {curation_results.content}")
        for idx, reference in enumerate(curation_results.references, 1):
            print(f"\n{idx}. 제목: {reference.title}")
            print(f"   작가: {reference.artist}")
            print(f"   시기: {reference.period}")
            print(f"   매체: {reference.medium}")
            print(f"   설명: {reference.description}")
