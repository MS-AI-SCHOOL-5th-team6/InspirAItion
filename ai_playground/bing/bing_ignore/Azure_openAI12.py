...
class AzureAIAssistant:
    ...
    def get_last_assistant_message(self, thread_id):
        ...
        try:
            ...
            logger.warning(f"스레드 {thread_id}에서 어시스턴트 메시지를 찾을 수 없음")
            return None
        except Exception as e:
            logger.error(f"메시지 가져오기 중 오류 발생: {e}")
            return None
    ...
    async def _search_reference_artworks(self, request: CurationRequest) -> Result[List[Dict]]:
        ...
        try:
            ...
            for result in search_results:
                ...
                if result.data.get("objectIDs"):
                    # 최대 3개의 작품 상세 정보 가져오기
                    object_ids = result.data["objectIDs"][:3]
                    artwork_tasks = [
                    ...
                    valid_details = [detail.data for detail in artwork_details if detail.success]
                    processed_results.extend(valid_details)
            ...
            return Result(success=False, error=f"참조 작품 검색 중 오류: {str(e)}")
    ...
    async def _generate_assistant_response(self, prompt: str) -> str:
        ...
        try:
            ...
            if self.gpt_client.wait_for_run_completion(thread.id, run.id):
                ...
                return "응답을 생성하는 데 실패했습니다."
            
            logger.error("런 완료에 실패했습니다.")
            return "응답을 생성하는 데 실패했습니다."
        except Exception as e:
...
async def main():
    async with AsyncCurationClient() as curation_service:
        request = CurationRequest(
            user_prompt="피카소 스타일로 서울 풍경을 그리고 싶어.",
            selected_style=CurationStyle.EMOTIONAL,
            image_analysis=ImageAnalysis(
                dense_caption="oil painting of a Seoul cityscape in the style of Picasso, with abstract forms, vibrant colors, and dynamic composition",
...
async def main():
    async with AsyncCurationClient() as curation_service:
        ...
        if result.success:
            ...
            client = AsyncOpenAI(api_key="YOUR_AZURE_OPENAI_KEY")  
            
            response = await client.chat.completions.create(
                model="gpt-4",
                messages=[
                    {"role": "system", "content": "Translate the following text into Korean."},
