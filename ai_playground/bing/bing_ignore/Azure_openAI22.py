...
AzureAIAssistant_api_version = os.getenv("AzureAIAssistant_api_version")
...
class MetMuseumClient(Tool):
    def __init__(self, session: aiohttp.ClientSession):
        super().__init__(session)
        self.base_url = os.getenv("MET_data_source")
    ...
    async def get_artwork_details(self, unified_input: UnifiedInput) -> Result:
        object_id = unified_input.get("object_id")
        return await self.make_request("GET", f"objects/{object_id}")

# 향상된 큐레이션 서비스
class EnhancedCurationService:
    def __init__(self, assistant, style_prompts):
        ...
        try:
            ...
            if not request.reference_search:
                ...
                for context_type in style_context:
                    ...
                    additional_context_tasks.append(self.assistant.call_action(search_query, limit=10))  # 결과 제한
    ...
    async def _fetch_context_data(self, search_query: str) -> Any:
        """컨텍스트 데이터를 가져오는 메서드"""
        try:
            async with self.session.get(f"{MET_data_source}/search?q={search_query}") as response:
                if response.status == 200:
                    data = await response.json()
                    return data
                else:
