...
connection_string = "eastus2.api.azureml.ms;b850d62a-25fe-4d3a-9697-ea40449528a9;5th_3rd_prj_team6;openai_o3"
...
class EnhancedCurationService:
    ...
    def _create_gpt_prompt(self, integrated_data: Dict) -> str:
        ...
        # 참조 작품과 컨텍스트 데이터를 프롬프트에 추가
        references_text = "\n".join([
            f"- {artwork['title']} by {artwork['artist']} ({artwork['period']})"
    ...
    async def _generate_gpt_curation(self, integrated_data: Dict) -> Result[str]:
        ...
        try:
            ...
            return Result(success=True, data=response.choices[0].message.content)
        except Exception as e:
            return Result(success=False, error=str(e))
    ...
    async def _fetch_context_data(self, search_query: str) -> Any:
        """컨텍스트 데이터를 가져오는 메서드"""
        try:
            async with self.session.get(f"https://api.example.com/search?q={search_query}") as response:
                if response.status == 200:
                    data = await response.json()
                    return data
                else:
...
class AsyncCurationClient:
    async def __aenter__(self):
        self.session = aiohttp.ClientSession()
        self.met_client = MetMuseumClient(self.session)
        self.gpt_client = Azure_openAI_client  # GPT 클라이언트 초기화
        self.curation_service = EnhancedCurationService(
            met_client=self.met_client,
            gpt_client=self.gpt_client,  # GPT 클라이언트 전달
            session=self.session
        )
        return self.curation_service
