...
from .models import AIImageGeneration, AIImageAnalysis, ArtworkAnalysis, ArtCuration
...
class MetMuseumClient(BaseAPIClient):
    def __init__(self, session: aiohttp.ClientSession):
        super().__init__(session, os.getenv("MET_data_source"))
...
class EnhancedCurationService:
    ...
    def _create_gpt_prompt(self, integrated_data: Dict) -> str:
        ...
        for artwork in reference_artworks:
            prompt += f"- {artwork.get('title', 'Untitled')} by {artwork.get('artist', 'Unknown')}\n"
        prompt += "\n추가 컨텍스트:\n"
...
# import nest_asyncio - Django에서 비동기 처리를 위한 설정
import nest_asyncio
nest_asyncio.apply()
